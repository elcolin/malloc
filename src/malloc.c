...
void *mmalloc(size_t size){
    ...
    if (!heap_lst) // If no heap create new heap
        heap_lst = allocate_new_heap((determine_heap_size(size) + 15) & ~15, get_heap_label_size(size));
    else
    ...
    if (!new_block)
    {
        ...
        if (!new_block)
            new_block = allocate_new_block(allocate_new_heap((determine_heap_size(size) + 15) & ~15, get_heap_label_size(size)), size);
    }
